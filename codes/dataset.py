...
class WestChaeVI_Dataset(Dataset):

  def __init__( self, root_path, mode, transforms):
     # root_path : '/content/drive/MyDrive/Segmentation/'    //    mode : train, valid, test   //  cls : bf_image, bf_label
    self.all_images = sorted(glob.glob(os.path.join(root_path, mode, 'bf_image', '*')))
    self.all_labels = sorted(glob.glob(os.path.join(root_path, mode, 'bf_label', '*')))
